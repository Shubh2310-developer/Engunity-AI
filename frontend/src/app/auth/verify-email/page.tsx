...
export default function VerifyEmailPage(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    const checkVerificationStatus = async () => {
      ...
      try {
        ...
        if (session?.user?.email_confirmed_at || isAlreadyVerified) {
    ...
    // Handle already verified users (OAuth or pre-verified)
    if (isAlreadyVerified || isOAuth) {
      setVerificationStatus('verified');
      setTimeout(() => {
        if (mounted) {
          router.push(redirectTo);
        }
      }, 2000); // Slightly longer delay for welcome message
      return;
    }

    // Initial check for regular users needing verification
  ...
  /**
   * Get status message based on verification state and user type
   */
      ...
      default:
        if (isOAuth) {
          return {
            ...
            description: 'Your OAuth account is being set up. You\'ll be redirected shortly.',
          };
        } else {
          return {
      ...
      <Card className="border-0 shadow-lg bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm">
        <CardHeader className="text-center pb-4">
          <CardTitle className="text-lg font-semibold text-gray-900 dark:text-white">
            {isOAuth ? 'Account Setup' : (isAlreadyVerified ? 'Welcome!' : 'Check your email')}
          </CardTitle>
          <CardDescription className="text-gray-600 dark:text-gray-400">
            {verificationStatus === 'pending' && !isOAuth && !isAlreadyVerified && 'Click the verification link to activate your account'}
            {verificationStatus === 'pending' && isOAuth && 'Setting up your OAuth account...'}
            {verificationStatus === 'pending' && isAlreadyVerified && 'Your account is ready to use'}
        ...
        <CardContent className="space-y-4">
          ...
          {/* Resend Section - only for email/password users */}
          {verificationStatus === 'pending' && !isOAuth && !isAlreadyVerified && (
          ...
          {verificationStatus === 'verified' && (
            <div className="text-center">
              ...
              >
                ...
                {isOAuth || isAlreadyVerified ? 'Continue to Dashboard' : 'Continue to Dashboard'}
              </Button>
            </div>
          )}
        </CardContent>
